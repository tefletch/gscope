...
static void process_query(search_t query_type)
{
    ...
    if (query_type <= FIND_INCLUDING) // if this is not a "virtual button" query
    {
        ...
        pattern = strdup("_");  // Dummy search pattern for "virtual button"
                                        ...
                                        gpointer         user_data)
{
    ...
    if ( !customized )
    {
        ...
        NO_FAIL(asprintf(&full_comment, "%s%s%s%s%d.%d.%d%s",
                              description,
                              based_on,
                              modified_by,
...
void
on_find_functions_called_by_button_clicked (GtkButton       *button,
                                            gpointer         user_data)
{
    if ( !search_button_lockout )
    {
...
void
on_find_functions_calling_button_clicked (GtkButton       *button,
                                          gpointer         user_data)
{
    if ( !search_button_lockout )
    {
                                        ...
                                        gpointer         user_data)
{
    ...
    if ( !initialized )
    {
        ...
        gtk_entry_set_text      (GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"editor_command_entry")), settings.fileEditor);
        ...
        gtk_entry_set_text      (GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"autogen_cache_path_entry")),  settings.autoGenPath);
        gtk_entry_set_max_length(GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"autogen_suffix_entry1")),     MAX_STRING_ARG_SIZE-1);
        gtk_entry_set_text      (GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"autogen_suffix_entry1")),     settings.autoGenSuffix);
        gtk_entry_set_max_length(GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"autogen_cmd_entry1")),        MAX_STRING_ARG_SIZE-1);
        gtk_entry_set_text      (GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"autogen_cmd_entry1")),        settings.autoGenCmd);
        ...
        gtk_entry_set_text      (GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"autogen_search_root_entry1")),settings.autoGenRoot);
        ...
        {
            gchar *markup_buf;
            NO_FAIL(asprintf(&markup_buf, "<span foreground=\"seagreen\" weight=\"bold\">%s</span>", settings.rcFile));
        ...
        gtk_entry_set_text      ( GTK_ENTRY(lookup_widget(GTK_WIDGET (prefs_dialog),"cref_entry")),       settings.refFile);
        gtk_entry_set_max_length( GTK_ENTRY(lookup_widget(GTK_WIDGET (prefs_dialog),"search_log_entry")), MAX_STRING_ARG_SIZE-1);
        gtk_entry_set_text      ( GTK_ENTRY(lookup_widget(GTK_WIDGET (prefs_dialog),"search_log_entry")), settings.searchLogFile);
        ...
        gtk_entry_set_text      (GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"terminal_app_entry")),settings.terminalApp);
        gtk_entry_set_max_length(GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"file_manager_app_entry")),MAX_STRING_ARG_SIZE-1);
        gtk_entry_set_text      (GTK_ENTRY (lookup_widget(GTK_WIDGET (prefs_dialog),"file_manager_app_entry")),settings.fileManager);
...
on_open_terminal(GtkWidget *menuitem, gchar *container)
{
    ...
    NO_FAIL(asprintf(&command, settings.terminalApp, coded_container));  // Synthesize the full command line
    my_system(command);
...
on_open_file_browser(GtkWidget *menuitem, gchar *container)
{
    ...
    NO_FAIL(asprintf(&command, settings.fileManager, coded_container));
    my_system(command);
...
void
on_open_quick_view(GtkWidget *menuitem, gchar *file_and_line)
{
    gchar *linenum;
...
void
show_context_menu(GtkWidget *treeview, GdkEventButton *event, gchar *filename, gchar *linenum)
{
    ...
    if (event->type == GDK_BUTTON_PRESS)  // Process single button click (ignore GDK_[23]BUTTON_PRESS event types)
    {
        switch (event->button)
        {
            ...
            case 3:     // right-button -- List item context menu
                ...
                if ( gtk_tree_view_get_path_at_pos( GTK_TREE_VIEW(widget), event->x, event->y, &path, NULL, NULL, NULL) )
                {
                    if ( DISPLAY_get_filename_and_lineinfo(path, &filename, &linenum) ) // Get the selected filename
                    {
                        // pass filename into show menu call
                        show_context_menu(widget, event, filename, linenum);
                    }
                    gtk_tree_path_free(path);
                }
                                        ...
                                        gpointer         user_data)
{
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter))
    {
        ...
        {
            // pass filename into show menu call
            show_context_menu(widget, NULL, filename, linenum);
        }
...
on_fileview_start_editor_activate(GtkMenuItem *menuitem, gpointer user_data)
{
    ...
    NO_FAIL(asprintf(&line_str,"%d", windowPtr->line));
...
void
on_fileview_close_activate(GtkMenuItem *menuitem, gpointer user_data)
{
...
void
on_retain_text_failed_search_checkbutton_toggled(GtkToggleButton *togglebutton,
                                                 gpointer         user_data)
{
    if (!initializing_prefs)
    {
...
gboolean
on_editor_command_entry_focus_out_event (GtkWidget       *widget,
                                        GdkEventFocus   *event,
                                        gpointer         user_data)
{
...
void
on_source_directory_browse_button_clicked (GtkButton       *button,
                                           gpointer         user_data)
{
...
gboolean
on_open_file_chooser_dialog_delete_event(GtkWidget *widget,
                                         GdkEvent *event,
                                         gpointer user_data)
{
    gtk_widget_hide(widget);
...
gboolean
on_output_file_chooser_dialog_delete_event(GtkWidget *widget,
                                           GdkEvent *event,
                                           gpointer user_data)
{
    gtk_widget_hide(widget);
...
gboolean
on_autogen_cache_path_entry_focus_out_event(GtkWidget       *widget,
                                        GdkEventFocus   *event,
                                        gpointer         user_data)
{
...
gboolean
on_autogen_search_root_entry1_focus_out_event(GtkWidget       *widget,
                                              GdkEventFocus   *event,
                                              gpointer         user_data)
{
...
gboolean
on_autogen_suffix_entry1_focus_out_event(GtkWidget       *widget,
                                         GdkEventFocus   *event,
                                         gpointer         user_data)
{
    const gchar *entry_text;
